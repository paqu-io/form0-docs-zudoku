...
1. Entra nella modalità di modifica dello schema:
   ...
   <Callout type="caution" title="Editor richiesto">
     La modalità di modifica dello schema apre il tuo editor di codice. Assicurati che la variabile
     `EDITOR` o `VISUAL` sia configurata: - **Linux/macOS**: Aggiungi `export EDITOR=code` a
     `~/.bashrc`, `~/.zshrc` o `~/.profile`, poi riavvia il terminale o esegui `source ~/.bashrc`. -
     **Windows (PowerShell)**: Esegui `$env:EDITOR = "code"` per la sessione corrente, oppure
     aggiungilo al tuo profilo PowerShell (`$PROFILE`). - **Windows (CMD)**: Usa `set EDITOR=code`
     per la sessione, oppure impostalo in modo permanente tramite _Proprietà del sistema → Variabili
     d'ambiente_.
   </Callout>
