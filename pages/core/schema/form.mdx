...
<Callout type="info" title="Application-level attributes">
  Attributes marked with ✓ in the **App** column are application-level metadata not processed directly by the engine.
  They can be used when integrating with [reform](https://reformapp.io) or when building similar functionality in your own application.
</Callout>

| Attribute | Type | Required | App | Notes |
| --- | :---: | :---: | :---: | --- |
| `name` | string | ✓ | | Human-friendly form name. |
| `description` | string | | | Optional form description. |
| `id` | string | ✓ | | Unique form identifier (UUID recommended). |
| `record_count` | number | | ✓ | Number of records in the form. |
| `record_last_change_at` | string | | ✓ | Last record change date/time (ISO 8601). |
...
| `main_org_id` | string | | ✓ | Main organization ID (or `personal`). |
...
| `location_enabled` | boolean | | ✓ | Enables location capture (if supported by client). |
...
| `form_links` | object | | | Links to/from other forms (used with FormLinkField). |
